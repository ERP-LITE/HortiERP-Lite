...
<template>
  <div>
    <PageHeader title="Perdas" subtitle="Registro de perdas com baixa automática no estoque">
      <template #actions>
        <BaseButton @click="openCreateModal">+ Registrar perda</BaseButton>
      </template>
    </PageHeader>
      ...
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="divide-y divide-gray-100">
          ...
          <tr v-else-if="losses.length === 0">
            <td colspan="5" class="px-4 py-6 text-center text-sm text-gray-500">Nenhuma perda registrada.</td>
          </tr>
          <tr v-for="loss in losses" v-else :key="loss.id">
            ...
            <td class="px-4 py-3 text-sm font-medium text-gray-900 whitespace-nowrap">{{ loss.product?.name }}</td>
            <td class="px-4 py-3 text-sm text-gray-700 whitespace-nowrap">{{ Number(loss.quantity) }}</td>
            <td class="px-4 py-3 whitespace-nowrap">
              <BaseBadge variant="danger">{{ reasonLabels[loss.reason] }}</BaseBadge>
            </td>
            <td class="px-4 py-3 text-sm text-gray-500 hidden sm:table-cell">{{ loss.notes || '—' }}</td>
          </tr>
        </tbody>
      </table>
    </div>
