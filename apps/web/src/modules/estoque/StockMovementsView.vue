      ...
      <table class="min-w-full divide-y divide-gray-200">
        <thead class="bg-gray-50">
          <tr>
            ...
            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Saldo após</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-gray-100">
          ...
          <tr v-else-if="movements.length === 0">
            <td colspan="5" class="px-4 py-6 text-center text-sm text-gray-500">Nenhuma movimentação registrada.</td>
          </tr>
          <tr v-for="movement in movements" v-else :key="movement.id">
            <td class="px-4 py-3 text-sm text-gray-500 whitespace-nowrap">{{ formatDateTime(movement.createdAt) }}</td>
            ...
            <td class="px-4 py-3 text-sm text-gray-700 whitespace-nowrap">{{ Number(movement.quantity) }}</td>
            <td class="px-4 py-3 text-sm text-gray-500 whitespace-nowrap">{{ Number(movement.balanceAfter) }}</td>
          </tr>
        </tbody>
      </table>
    </div>
